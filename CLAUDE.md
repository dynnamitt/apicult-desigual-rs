...
- **`wasm`** (feature-gated) — `wasm-bindgen` surface for the web demo. Exposes `WasmLayout` (opaque handle wrapping `HGridLayout`) with `tris()` (flat `n*9`, gap_tris + hex_face_tris — *not* `all_tris`, to avoid double-counting against quads), `quads()` (flat `n*12` from `gap_quads`), and `borderline_cells(VertexDir)`. Plus a one-shot `generate_geometry(radius, h_seed, r_seed, overrides) -> Geometry { tris, quads }`. Inputs flow through `OverrideSpec { kind, q, r, value }` (mapped to `Override`), `NoiseChannel` (mirrors `NoiseType`), `VertexDir` (mirrors `hexx::VertexDirection` indices East..NorthEast). Outputs flow through `HexCellView { q, r, height, radius }` (Hex decomposed since foreign types can't carry `#[wasm_bindgen]`).
...
`make preview` runs `geo_export` three times (plain SVG, rich SVG, JSON v1 — all using `HSEED`), runs `wasm-pack build` once to produce `web/pkg/`, then templates `web/svg-preview.html` (with the short git SHA) and `web/hex-terrain.html` (with the short SHA + `RADIUS`) into `target/svg-preview/`. The terrain page imports `pkg/apicult_desigual.js` and calls `generate_geometry(RADIUS, h_seed, r_seed, [])` once per mesh at page load with fresh random u32 seeds, so each refresh produces a visually different multi-mesh layout — no JSON files involved at runtime. The SVG previews and `apicult-desigual.json` (v1) still share `HSEED` for grid/mesh consistency; pin it with `make HSEED=N preview` for a reproducible build (the chosen seed is echoed at the end). The `.github/workflows/svg-preview.yml` CI installs the `wasm32-unknown-unknown` target and `wasm-pack`, runs the same `make` target on push to `main`, and publishes the result to the `gh-pages` branch — so the live demo at https://dynnamitt.github.io/apicult-desigual-rs/ tracks `main` automatically.
