...
- **`wasm`** (feature-gated) — `wasm-bindgen` surface for the web demo. Exposes `WasmLayout` (opaque handle wrapping `HGridLayout`) with four renderable flat-buffer methods plus one structured query, all the renderables filtered by `entangled: bool`:
  ...
  - `face_bridge_quads(entangled)` — per-hex face-edge bridge quads grouped `n_hexes * 78` floats (6 edge slots × 13 floats: `[flag, v0, n0, n1, v1]`). `flag` is `1.0` when the neighbor on that face edge exists in-grid, `0.0` for border edges (followed by 12 sentinel zeros). Same hex ordering as `face_tris`; corner layout matches `gap_quads` (`v0`/`v1` on source hex, `n0`/`n1` on neighbor). Lets the bridge band shader render a chosen face-edge's gap quad as a directional continuation of the source hex's ring overlay.
...
- **wires** — `wireframe` (Line2 dashed) toggle plus `line width` and `dash speed` sliders. `dash speed` also drives the `glowy hexes` shader (which lives in the **shaders** fieldset but reads the same `state.dashSpeed`).
- **shaders** — `glowy hexes` (`#btn-shader` — GLSL dashed bloom-driven rim segments along gap quads), `ringed-entities`, and `bridge` toggles. The latter two are keyed off the same randomly-picked ~10% subset of hex face fans (one selection per cluster build, shared by both overlays); `glowy hexes` is independent.
  - `ringed-entities` (`createBandShader`): concentric color rings on the picked hexes, base color (`FILL_COLORS[index]`) at the rim fading to `BG_COLOR` at the centroid via a per-vertex barycentric weight (`aWeight = 1.0` at fan-center vertex, `0.0` at perimeter corners; smooth varying interpolation + `floor(vWeight * uBands) / (uBands - 1.0)` quantizes into 5 steps). A `uBrightness` uniform pre-multiplies the band color by `ambient + sun_intensity * max(0, sun_dir.y)` so the overlay matches the lit `MeshStandardMaterial` fill — valid because flat-top hex face fans have a constant normal `(0, 1, 0)`.
